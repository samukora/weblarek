...
export class FormContacts extends Form<IFormContacts> {
  ...
  constructor(container: HTMLElement, actions: IContactsActions) {
    ...
    this.emailElement.addEventListener('keyup', debounce(actions.onContactsChange, 300));

    this.phoneElement = ensureElement<HTMLElement>("input[name='phone']", this.container);
    this.phoneElement.addEventListener('keyup', debounce(actions.onContactsChange, 300));
  }
}